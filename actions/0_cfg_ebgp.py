...
class InternalCfgeBGP(Action):
    def run(self, deviceIP, cmd_path, neighborIP, localAS, remoteAS):
        ...
        url = url_base + "/set/protocols/bgp/" + str(localAS) + "/neighbor/" + neighborIP + "/remote-as/" + str(remoteAS)
        url_list.append(url)

        url = url_base + "/set/protocols/bgp/" + str(localAS) + "/neighbor/" + neighborIP + "/address-family/ipv4-unicast"
        url_list.append(url)
