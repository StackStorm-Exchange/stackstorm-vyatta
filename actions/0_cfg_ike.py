...
class InternalCfgIpRoute(Action):
    def run(self, deviceIP, cmd_path, proposal, ikeGroup, ikeVersion, encryption, hashAlg, lifetime):
        ...
        url_1 = "/set/security/vpn/ipsec/ike-group/" + ikeGroup
        url_base = url_0 + url_1

        url = url_base + "/proposal/" + proposal
        url_list.append(url)

        url = url_base + "/ike-version/" + ikeVersion
        url_list.append(url)

        url = url_base + "/proposal/" + proposal + "/encryption/" + encryption
        url_list.append(url)

        url = url_base + "/proposal/" + proposal + "/hash/" + hashAlg
        url_list.append(url)

        url = url_base + "/lifetime/" + lifetime
        url_list.append(url)
